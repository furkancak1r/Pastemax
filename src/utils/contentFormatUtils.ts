...
interface FormatContentParams {
  ...
  includeAllFileTree: boolean; // Whether to include the entire workspace file tree
  includeFileTree: boolean; // Whether to include file tree of selected files in output
...
export const formatBaseFileContent = ({
  ...
  if ((includeAllFileTree || includeFileTree) && selectedFolder) {
    const normalizedFolder = normalizePath(selectedFolder);
    const filesForTree = includeAllFileTree ? files : sortedSelected;
