...
import { Workspace, SelectionSet } from './types/WorkspaceTypes';
...
const App = (): JSX.Element => {
  ...
  const [includeAllFileTree, setIncludeAllFileTree] = useState(true);
  ...
  const handleSaveSelectionSet = useCallback(
    (name: string) => {
      if (!name.trim() || selectedFiles.length === 0 || !currentWorkspaceId) {
        if (!currentWorkspaceId) {
      ...
      const newSelectionSet: SelectionSet = {
        id: `selection-set-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
        name: name.trim(),
      ...
      setWorkspaces((prevWorkspaces) => {
        const updatedWorkspaces = prevWorkspaces.map((workspace) => {
          if (workspace.id === currentWorkspaceId) {
            const updatedSelectionSets = [...(workspace.selectionSets || []), newSelectionSet];
            return { ...workspace, selectionSets: updatedSelectionSets };
          }
          return workspace;
        });
        return updatedWorkspaces;
    ...
    [selectedFiles, currentWorkspaceId, setWorkspaces]
  );

  const handleLoadSelectionSet = useCallback((filePaths: string[]) => {
    setSelectedFiles((prevSelected) => {
      const newSelected = new Set([
        ...prevSelected.map(normalizePath),
        ...filePaths.map(normalizePath),
      ]);
      return Array.from(newSelected);
    });
  }, []);

  const handleDeleteSelectionSet = useCallback(
    (selectionSetId: string) => {
      if (!currentWorkspaceId) return;

      if (!window.confirm('Are you sure you want to delete this selection set?')) {
        return;
      }
      ...
      setWorkspaces((prevWorkspaces) => {
        return prevWorkspaces.map((workspace) => {
          if (workspace.id === currentWorkspaceId) {
            const updatedSelectionSets = (workspace.selectionSets || []).filter(
              (set) => set.id !== selectionSetId
            );
            return { ...workspace, selectionSets: updatedSelectionSets };
          }
          return workspace;
  ...
  const handleTaskTypeChange = (taskTypeId: string) => {
    ...
    const selectedTask = allTaskTypes.find((type) => type.id === taskTypeId);
    if (selectedTask) {
      setUserInstructions(selectedTask.prompt);
    } else {
      setUserInstructions('');
    }
  };
    ...
    <ThemeProvider>
      <div className="app-container">
        ...
        <div className="main-content">
          ...
          {selectedFolder ? (
            <Sidebar
              ...
              onSaveSelection={() => setIsSaveSelectionModalOpen(true)}
              onLoadSelectionSet={handleLoadSelectionSet}
              onDeleteSelectionSet={handleDeleteSelectionSet}
            />
          ) : (
            ...
            <div className="copy-settings-container">
              <div className="copy-settings-options">
                <div
                  ...
                  title="Include the entire file tree of the workspace in the copyable content"
                >
                  <ToggleSwitch
                    id="includeAllFileTree"
                    checked={includeAllFileTree}
                    onChange={(e) => {
                      setIncludeAllFileTree(e.target.checked);
                      if (e.target.checked) {
                        setIncludeFileTree(false);
                      }
                    }}
                  />
                  <label htmlFor="includeAllFileTree">Include All File Tree</label>
                </div>
                <div
                  className="toggle-option-item"
                  title="Include File Tree of selected files in the Copyable Content"
                >
                  <ToggleSwitch
                    id="includeFileTree"
                    checked={includeFileTree}
                    onChange={(e) => {
                      setIncludeFileTree(e.target.checked);
                      if (e.target.checked) {
                        setIncludeAllFileTree(false);
                      }
                    }}
                  />
        ...
        <SaveSelectionModal
          ...
          onClose={() => setIsSaveSelectionModalOpen(false)}
          onSave={handleSaveSelectionSet}
        />
      </div>
    </ThemeProvider>
  );
